...
async function executeFootballHourlySync() {
  if (isRunning) {
    logger.warn('Football hourly sync already in progress, skipping...');
    return;
  }
  ...
  try {
    ...
    logger.info('Starting hourly football (NFL/NCAAF) sync...');

    const startTime = Date.now();
    const result = await statsSyncService.syncFootballHourlyWindow({
    ...
    if (result.errors.length === 0) {
      logger.info(`✅ Football hourly sync completed in ${duration}ms`);
      ...
      logger.error(`❌ Football hourly sync completed with ${result.errors.length} errors in ${duration}ms`);
    ...
    logger.error('Fatal error during football hourly sync:', error);
  } finally {
    isRunning = false;
  }
}

export function startFootballHourlySyncJob() {
  if (!env.API_SPORTS_KEY) {
    logger.warn('API_SPORTS_KEY not configured, football hourly sync job disabled');
    return null;
  }
  ...
  logger.info('Running initial football hourly sync...');
  executeFootballHourlySync();

  return task;
}

export function getFootballHourlySyncStatus() {
  return {
    isRunning,
    lastRunTime,
    lastResult,
    cronExpression: env.FOOTBALL_SYNC_CRON,
  };
}
